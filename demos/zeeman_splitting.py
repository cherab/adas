...
be_density = GaussianVolume(0.01 * ion_density, sigma * 10000)
...
be1_distribution = Maxwellian(be_density, temperature, bulk_velocity, beryllium_mass)
...
plasma.composition = [d0_species, d1_species, be1_species]
...
plt.title(r'D$\alpha$ spectrum observed at different angles between ray and magnetic field')
...
for angle in angles:
    angle_rad = deg2rad(angle)
    r = Ray(origin=Point3D(0, -5 * sin(angle_rad), -5 * cos(angle_rad)), direction=Vector3D(0, sin(angle_rad), cos(angle_rad)),
            min_wavelength=526.9, max_wavelength=527.9, bins=500)
    multiplet.append(r.trace(world))
...
plt.xlabel('Wavelength (nm)')
plt.ylabel('Radiance (W/m^2/str/nm)')
plt.title('Be II 4s1 2s0.5 -> 3p1 2p2.5 spectrum observed at different angles between ray and magnetic field')
